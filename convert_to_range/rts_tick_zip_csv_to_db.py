...
def zip_csv_convert_to_db(pair_lst, quantity_bars):
    ...
    for index, pair in enumerate(pair_lst):
        ...
        # Чтение данных из пары файлов в DF
        df_tick_0 = pd.read_csv(pair[0], compression='zip')
        df_tick_1 = pd.read_csv(pair[1], compression='zip')

        # Удаляем дубликаты строк по 'datetime', оставляя только первое вхождение.
        df_tick_0 = df_tick_0.drop_duplicates(subset='datetime', keep='first')
        df_tick_1 = df_tick_1.drop_duplicates(subset='datetime', keep='first')
        ...
        size_dic[end_range_size - 50] = len(create_range_bars(df_tick_0, end_range_size - 50))
        size_dic[end_range_size + 50] = len(create_range_bars(df_tick_0, end_range_size + 50))
        size_dic[end_range_size - 100] = len(create_range_bars(df_tick_0, end_range_size - 100))
        size_dic[end_range_size + 100] = len(create_range_bars(df_tick_0, end_range_size + 100))
        ...
        df = create_range_bars(df_tick_1, closest_key)
...
if __name__ == "__main__":
    ...
    pair_lst = []
    for file_0, file_1 in zip(files_with_paths, files_with_paths[1:]):
        pair_lst.append([file_0, file_1])
